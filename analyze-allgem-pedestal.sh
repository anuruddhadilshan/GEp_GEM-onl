...
# Script to run pedestal analysis for all GEp gems and display, print, and log the results.

# Usage: $ analyze-allgem-pedestal <runnunm> <daqtype>
...
ROOTFILE=$ROOT_DIR/${fname_prefix}_replayed_allGEMs_${runnum}_seg0_${firstsegment}_firstevent${firstevent}_nevent${nevents}.root
...
if yes_or_no "Copy ped and CM files to vtp directories and SBS-replay/DB/gemped?"; then
    ...
    echo "Then change db_sbs.gemFT.dat and db_sbs.gemFPP.dat to also match the pedestal run number"
    echo ""
else
